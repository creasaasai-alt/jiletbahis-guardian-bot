...
const resolvers = Object.entries(ISP_SERVERS).map(([name, ip]) => {
    const r = new dns_1.default.promises.Resolver();
    r.setServers([ip]);
...
class DnsService {
     ...
     */
    static async isDomainBlockedByBTK(domain) {
        try {
     ...
     */
    static async isDomainActive(domain) {
        try {
     ...
     */
    static async isServerHealthy(domain) {
        try {
            // Sadece sayfanın başlığını (HEAD) çekeriz, tüm siteyi indirip yormayız.
            await axios_1.default.head(`https://${domain}`, { timeout: 7000 });
            return true;
        }
        catch (error) {
