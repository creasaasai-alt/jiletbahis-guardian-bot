...
// --- BİLDİRİM ŞABLONLARI ---
const sendBlockedAlert = async (oldDomain, targetDomain) => {
...
const sendDelayAlert = async (targetDomain) => {
...
const sendSuccessAlert = async (newDomain) => {
...
const sendServerRecoveryAlert = async (domain) => {
...
node_cron_1.default.schedule('* * * * *', async () => {
    const state = domain_service_1.DomainService.getState();
    const now = new Date();
    try {
        if (state.currentState === 'OK' || state.currentState === 'SERVER_DOWN') {
            // 1. Önce BTK Engeli Var Mı? (En Önemlisi)
            const isBlocked = await dns_service_1.DnsService.isDomainBlockedByBTK(state.currentDomain);
            if (isBlocked) {
                console.log(`🚨 DİKKAT: ${state.currentDomain} BTK tarafından engellendi!`);
                const nextDomain = domain_service_1.DomainService.getNextDomain(state.currentDomain, 1);
                state.currentState = 'PENDING';
                state.pendingDomain = nextDomain; // Sadece log/info için
                state.blockedAt = now.toISOString();
                domain_service_1.DomainService.saveState(state);
            ...
            const isHealthy = await dns_service_1.DnsService.isServerHealthy(state.currentDomain);
            ...
            for (let i = 1; i <= 20; i++) {
                const testDomain = domain_service_1.DomainService.getNextDomain(state.currentDomain, i);
                // Hem DNS aktif mi (BTK yememiş ve IP dönüyor) hem de sunucu 200 dönüyor mu?
                if (await dns_service_1.DnsService.isDomainActive(testDomain) && await dns_service_1.DnsService.isServerHealthy(testDomain)) {
            ...
            if (foundActiveDomain) {
                ...
                if (diffMinutes >= 15 && state.currentState === 'PENDING') {
                    ...
                    const expectedNext = domain_service_1.DomainService.getNextDomain(state.currentDomain, 1);
...
bot.command('testengel', async (ctx) => {
    ...
    const next = domain_service_1.DomainService.getNextDomain(current);
...
const initializeSystem = async () => {
    ...
    let isBlocked = await dns_service_1.DnsService.isDomainBlockedByBTK(current);
    while (isBlocked) {
        console.log(`❌ ${current} engelli. Bir sonrakine bakılıyor...`);
        current = domain_service_1.DomainService.getNextDomain(current);
        isBlocked = await dns_service_1.DnsService.isDomainBlockedByBTK(current);
    }
